/*
  TANDARA DIAGRAM ENGINE — COMMON STANDARD 2026-09-18
  Jedini zajednički tehnički motor za svih 5 HR + 5 EN dijagrama.
  Rodoslovni podaci ne pripadaju ovom motoru.
  Funkcije: grane +/−, više brakova i djeca po majci, četveropoljska pretraga,
  zoom/pan, fullscreen, ispis, kartica osobe, HR/EN, N.G., privremeni zapisi
  nepoznatog spola te kći prenositeljica prezimena.
*/
(() => {
  'use strict';
  ...
  if (!dataEl) {
    console.error('TANDARA diagram: nedostaje #diagramData.');
    return;
  }
  ...
  try {
    ...
    console.error('TANDARA diagram: neispravni podaci dijagrama.', error);
    return;
  }
  ...
  function render() {
    ...
    for (const item of visible) {
      ...
      if (rec.carrierDaughter) classes.push('carrier-daughter');
      if (rec.type === 'unknown') classes.push('unknown');
      ...
      for (const extraCode of orderedFamilyExtras(item.code)) {
        ...
        const allowedSideTypes = new Set(['spouse', 'daughter', 'son', 'unknown']);
        const type = allowedSideTypes.has(sideRec.type) ? sideRec.type : 'daughter';
        const sideClasses = ['side-node', type];
        if (sideRec.carrierDaughter) sideClasses.push('carrier-daughter');
...
/* Zajednički sloj za skrivanje zaglavlja i sinkronizaciju fullscreen oznake. */
/* Zajednički sloj za skrivanje zaglavlja i stanje širine dijagrama. */
(() => {
  'use strict';
